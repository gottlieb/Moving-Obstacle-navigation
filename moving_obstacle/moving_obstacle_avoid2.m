function moving_obstacle_avoid2( filename, moviename )
...
p = [-2,0];  % position of robot
v = [2,.5];  % velocity of robot
...
for i=1:n
    ...
    hcv(i) = quiver(MOP(i,1), MOP(i,2), MOV(i, 1), MOV(i, 2)); % Draw initial obstacle velocity vector
    set(hcv(i),'Erasemode','normal');
end
...
for i=1:n
    ...
    offsetX = COP(i,1) + COV(i,3)*cos(COV(i,2));
    offsetY = COP(i,2) + COV(i,3)*sin(COV(i,2));
    hoc(i) = fill( ox+offsetX, oy+offsetY, 'k' ); % draw circular movement obstacles
    ...
    hccv(i) = quiver(offsetX, offsetY, COV(i,1)*cos(COV(i,2)-pi/2), COV(i,1)*sin(COV(i,2)-pi/2)); % Initial object velocity vector
    set(hccv(i),'Erasemode','normal');    
end
...
for k=1:400
    ...
    set(hva, 'XData', p(end,1), 'YData', p(end,2), 'UData', v(end,1), 'VData', v(end,2));
    ...
    for j=1:n
        ...
        set(hcv(j), 'Xdata', MOP(j,1), 'YData', MOP(j,2)); % Update obstacle velocity arrow
        if verbose && mod(k,2)==1
            plot( ox, oy, 'k' );
        end
        % find absolute Colision Cone
    ...
    for j=1:l
        ...
        set(hccv(i),'Xdata',cx, 'Ydata',cy, 'Udata', COV(j,1)*cos(COV(j,2)-pi/2), 'Vdata', COV(j,1)*sin(COV(j,2)-pi/2) );   % Update obstacle velocity arrow    
        if verbose && mod(k,2)==1
            plot( ox, oy, 'k' );
        end
        % find absolute Colision Cone
