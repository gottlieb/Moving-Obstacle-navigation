function [J] = cost_static_obs( v, p, thcl, thcr, po, dt )

%% cost_static_obs
% cost function of the robot at the location of 'p' and moving with velocity 
% 'v', against obstcle at the location of 'po ' such that collision cone is
...
%
    ...
    for i=1:n
        ...
        v_right_pl = (v(i,2)-p(2))*(pl(1)-p(1)) - (v(i,1)-p(1))*(pl(2)-p(2)) < 0;
        v_left_pr = (v(i,2)-p(2))*(pr(1)-p(1)) - (v(i,1)-p(1))*(pr(2)-p(2)) > 0;
        if v_right_pl & v_left_pr
            % v(i) is within VO
            [x,y] = line_intersection( p(1), p(2), pf(1), pf(2), po(1,1), po(1,2), po(2,1), po(2,2) );
            J(i) = sqrt( absV(1)*absV(1) + absV(2)*absV(2) ) / sqrt( (p(1)-x)^2 + (p(2)-y)^2 );      % inverse of collision time
        else
